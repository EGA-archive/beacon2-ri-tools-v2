...
def generate(dict_properties,list_of_headers, args):
    #os.path.join(args.input, 'datasets.csv') = conf.os.path.join(args.input, 'datasets.csv')
    total_dict =[]

    with open(os.path.join(args.input, 'datasets.csv'), 'r' ) as theFile:
        ...
        for line in reader:
            ...
            for kline, vline in line.items():
                ...
                if property_value not in list_of_headers:
                    raise Exception(('the header {} is not allowed. Please, take a look at files/headers/datasets.txt to check the headers allowed.').format(property_value))
...
output = os.path.join(args.output, 'datasets.json')
